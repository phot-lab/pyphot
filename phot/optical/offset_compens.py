...
def freq_offset_compensation(signals, sampling_rate):
    ...
    input_complex_signal_x = signals[0]
    input_complex_signal_y = signals[1]
    ...
    # frequency_scale = np.arange(-sampling_rate / 8, sampling_rate / 8, sampling_rate / fft_size / 4).reshape(
    #     (-1, 1))  # Hz  列出信号的频率坐标

    # Hz  列出信号的频率坐标
    frequency_scale = np.linspace(-sampling_rate / 8, sampling_rate / 8, fft_size + 1)
    ...
    phase_fre_offset = 2 * np.pi * fre_offset / sampling_rate  # rad  利用公式计算频偏带来的相位
    foc_vector = np.reshape(
        np.conj(np.arange(1, len(input_complex_signal_x) + 1)).T * phase_fre_offset, (-1, 1)
    )  # 对信号添加频偏带来的相位
