import numpy as np
from ..values import globals, constants
import math
...
class RxFrontend:
    ...
    def receive(self, lightwave: Lightwave) -> np.ndarray:
        ...
        if self.disp_accum is not None:
            hf = _post_fiber(self.disp_accum, self.slope_accum, self._lam, self._lam, lightwave.lam)
            hf = hf * dsp.my_filter(self.filter_type, freq_norm, 0.5 * self.bandwidth, self.optic_param)
        else:
            hf = dsp.my_filter(self.filter_type, freq_norm, 0.5 * self.bandwidth, self.optic_param)
    ...
    def _optic2elec(self, lightwave: Lightwave, num_pts) -> np.ndarray:
        ...
        if self.mod_format == "ook":
            ...
            delay = mathutils.n_mod(
                np.arange(num_fft) - round(self.mz_delay * num_pts), num_fft
...
def _post_fiber(disp_acc, slope_acc, lam, lam_rx, lam_central) -> np.ndarray:
    ...
    b30z = (
        np.square((lam / 2 / math.pi / LIGHT_SPEED)) * (2 * lam * disp_acc + np.square(lam) * slope_acc) * 1e-3
    )  # beta3*z [ns^3] @ lam0
