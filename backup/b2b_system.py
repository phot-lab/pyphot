...
if __name__ == "__main__":
    ...
    time_idx, rrc_filter = rrcosfilter(
        N=128 * up_sampling_factor, alpha=RRC_ROLL_OFF, Ts=1 / total_baud, Fs=sampling_rate
    ...
    phase_noise = phot.linewidth_induced_noise(
        len(tx_samples_x_dac), sampling_rate / total_baud, linewidth_tx, total_baud
    ...
    phase_carrier_offset = (
        np.arange(1, len(rx_signal_x) + 1).T * 2 * np.pi * frequency_offset / sampling_rate
    ).reshape((-1, 1))
    ...
    # 对信号采用CMA-RDE进行自适应均衡
    equalization_matrix_x, equalization_matrix_y = phot.cma_rde(
        input_x_i,
        input_x_q,
        input_y_i,
        input_y_q,
    ...
    # BPS算法
    equalization_matrix_x, phase_x = phot.bps_hybrid_qam(
        np.real(equalization_matrix_x), np.imag(equalization_matrix_x), num_test_angle, block_size, bits_per_symbol
    )
    equalization_matrix_y, phase_y = phot.bps_hybrid_qam(
        np.real(equalization_matrix_y), np.imag(equalization_matrix_y), num_test_angle, block_size, bits_per_symbol
    )
