...
if __name__ == "__main__":
    ...
    num_steps_list = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50]
    times = []
    for num_spans in num_steps_list:
        start = time.time()
        phot.fiber(signals, sampling_rate, num_spans, beta2, delta_z, gamma, alpha, span_length)
        duration = time.time() - start
        times.append(f"{duration:.2f}")
        print(f"Finished span: {num_spans}, time: {duration:.2f} s")

    df = pd.DataFrame({"num_spans": num_steps_list, "time (s)": times})
