...
if __name__ == "__main__":
    ...
    span_length_list = [75, 150, 300, 500]
    times = []
    for span_length in span_length_list:
        start = time.time()
        phot.fiber(signals, sampling_rate, num_spans, beta2, delta_z, gamma, alpha, span_length)
        duration = time.time() - start
        times.append(f"{duration:.2f}")
        print(f"Finished span_length: {span_length}, time: {duration:.2f} s")

    df = pd.DataFrame({"span_length (km)": span_length_list, "time (s)": times})
